...
plt.plot(processors, ideal_times, label="Perfect scaling")
...
for decomposition, launch_type, color, marker in zip(["1D", "2D"], ["Hybrid", "MPI"], list(mcolors.TABLEAU_COLORS.keys())[1:], ["s", "D"]):
    ...
    for domain_size, marker_fill in zip([256, 4096], ["none", "full"]):
        ...
        plt.plot(df_domain["mpi_procs"], df_domain["iteration_time"], label=f"{launch_type} {decomposition} {domain_size}x{domain_size} RMA", marker=marker, markersize=8, color=color, fillstyle=marker_fill, linestyle="--")
